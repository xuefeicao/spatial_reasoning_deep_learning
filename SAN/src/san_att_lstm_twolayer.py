...
from san_att_lstm_twolayer_theano_1 import *
...
options = OrderedDict()
# data related
options['data_path'] = '/media/data/xuefei/data_vqa'
options['feature_file'] = 'trainval_feat.h5'
#pickle file?
options['sal_file'] = 'trainval_sal.pkl'
#options['expt_folder'] = 'expt_1'
options['expt_folder']='/media/data/xuefei/data_vqa/saved_model/'
...
options['gamma'] = numpy.float32(0.9)
options['step'] = 10.0
options['step_start'] = 0
...
def get_lr(options, curr_epoch):
    if options['optimization'] == 'sgd':
        ...
        return numpy.float32(options['lr'] * (options['gamma'] ** power))  #
    else:
        return options['lr']
...
def train(options):
    ...
    data_provision_att_vqa = DataProvisionAttVqa(options['data_path'],
                                                 options['feature_file'],options['sal_file'])
    ...
    file_name=os.listdir(options['expt_folder'])
    options,params,shared_params=load_model(options['expt_folder']+file_name[0])
    options['lr'] = numpy.float32(0.005)
    options['gamma'] = numpy.float32(1)
    image_feat, prob_saliency, input_idx, input_mask, \
    ...
    f_grad_clip = theano.function(inputs = [],
                                  updates = update_clip,allow_input_downcast='True')
    f_output_grad_norm = theano.function(inputs = [],
                                         outputs = grad_norm,allow_input_downcast='True')
    f_train = theano.function(inputs = [image_feat, prob_saliency,input_idx, input_mask, label],
                              outputs = [cost, accu],
                              updates = update_grad,
                              on_unused_input='warn',allow_input_downcast='True')
    # validation function no gradient updates
    f_val = theano.function(inputs = [image_feat,prob_saliency, input_idx, input_mask, label],
                            outputs = [cost, accu],
                            on_unused_input='warn',allow_input_downcast='True')
    ...
    for itr in xrange(max_iters + 1):
        if (itr % eval_interval_in_iters) == 0 or (itr == max_iters):
            ...
            for batch_image_feat, batch_prob_saliency, batch_question, batch_answer_label \
                ...
                batch_image_feat = reshape_image_feat(batch_image_feat,
                                                      options['num_region'],
                                                      options['region_dim'])
                batch_prob_saliency=batch_prob_saliency.reshape(-1,196)
                [cost, accu] = f_val(batch_image_feat, batch_prob_saliency,input_idx, input_mask,
            ...
            if best_val_accu < ave_val_accu:
                ...
                files = glob.glob(options['expt_folder']+'*')
                for f in files:
                    os.remove(f)
                ...
                options['lr'] = get_lr(options, itr / float(num_iters_one_epoch))
            logger.info('validation cost: %f accu: %f' %(ave_val_cost, ave_val_accu))

        dropout.set_value(numpy.float32(1.))
        if options['sample_answer']:
            batch_image_feat, batch_prob_saliency,batch_question, batch_answer_label \
        ...
        [cost, accu] = f_train(batch_image_feat, batch_prob_saliency, input_idx, input_mask,
                               batch_answer_label.astype('int32').flatten())
