...
def init_params(options):
    ...
    #saliency parameters
    params = init_ff1layer(params,options['num_region'],options['region_dim'],options,prefix='saliency_param')
...
def init_fflayer(params, nin, nout, options, prefix='ff'):
    ...
    return params
#nin can change here    
def init_ff1layer(params, nin, nout, options, prefix='ff1'):
    ''' initialize ff layer
    '''
    params[prefix + '_w1'] = init_weight(nout,1, options)
    params[prefix + '_b2'] = np.zeros(1, dtype='float32')
    params[prefix + '_w2'] = init_weight(nin, 1, options)
    return params
...
def ff1layer(shared_params,x,options,prefix='ff1',act_func='sigmoid'):
    return eval(act_func)(T.dot(T.reshape(T.dot(x,shared_params[prefix + '_w1']),(-1,196)),shared_params[prefix + '_w2'])+
        shared_params[prefix + '_b2'])
...
def build_model(shared_params, options):
    ...
    prob_saliency=T.fmatrix('prob_saliency')
    # T x batch_size
    ...
    h_encode = h_encode[-1]
    weight_bt=ff1layer(shared_params,image_feat,options,prefix='saliency_param',
        act_func=options.get('saliency_params_act','sigmoid'))
    weight_bt=T.reshape(weight_bt,(-1,1))
    ...
    #use argmax will induce nondifferentiable result, like hard attention(potentially useful)
    prob_attention_1=prob_attention_1*(weight_bt)+prob_saliency*(1-weight_bt)
    prob_attention_1=T.reshape(prob_attention_1,(-1,196,1))
    #print prob_attention_1.shape.eval()
    ...
    return image_feat, prob_saliency, input_idx, input_mask, \
