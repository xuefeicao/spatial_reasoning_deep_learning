...
def init_ff1layer(params, nin, nout, options, prefix='ff1'):
    ''' initialize ff layer
    '''
    params[prefix + '_w1'] = init_weight(nout,nin, options)
    params[prefix + '_b1'] = np.zeros(nin, dtype='float32')
...
def ff1layer(shared_params,x,options,prefix='ff1',act_func='sigmoid'):
    return eval(act_func)(T.dot(T.dot(x,shared_params[prefix + '_w1'])+
        shared_params[prefix + '_b1'],shared_params[prefix + '_w2']))
...
def build_model(shared_params, options):
    ...
    weight_bt=T.reshape(weight_bt,(1,-1,196))
