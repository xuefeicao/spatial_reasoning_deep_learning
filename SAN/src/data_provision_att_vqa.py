...
class DataProvisionAttVqa:
    def __init__(self, data_folder, feature_file, sal_file):
        self._image_feat = self.load_image_feat(data_folder, feature_file)
        self._prob_saliency= self.load_prob_saliency(data_folder, sal_file)
    ...
    def load_prob_saliency(self, data_path, h5_file):
        sal_h5 = pkl.load(open(os.path.join(data_path, h5_file), 'r'))['sal_features']
        return sal_h5
    ...
    def iterate_batch(self, partition, batch_size):
        ...
        while current + batch_size <= self._question[partition].shape[0]:
            ...
            batch_prob_saliency= self._prob_saliency[batch_image_id]
            #batch_prob_saliency = batch_prob_saliency.todense()
            batch_answer_label = self._answer_label[partition][current :
                                                               current + batch_size]
            yield batch_image_feat,batch_prob_saliency, batch_question, batch_answer_label
    ...
    def iterate_batch_with_counter_imageid(self, partition, batch_size):
        ...
        while current + batch_size <= self._question[partition].shape[0]:
            ...
            batch_answer_counter = self._answer_counter[partition][current :
                                                                   current + batch_size]
            yield batch_image_id, batch_image_feat, batch_prob_saliency, batch_question, batch_answer_label, \
                batch_answer_counter
            current = current + batch_size
            logger.debug('iterating batch at current: %d'%(current))
        if current != self._question[partition].shape[0]:
    ...
    def next_batch(self, partition, batch_size):
        if self._pointer[partition] + batch_size <= self._question[partition].shape[0]:
            ...
            batch_image_feat = batch_image_feat.todense()
            batch_prob_saliency= self._prob_saliency[batch_image_id]
            print batch_image_feat.shape, batch_prob_saliency.shape
            #batch_prob_saliency = batch_prob_saliency.todense()
            ...
            return batch_image_feat, batch_prob_saliency, batch_question, batch_answer_label
        else:
