...
class Attention:
    ...
    def build(self,rgb,enc_size,read_n,T,output_shape,train_mode,conv_layer=True):
        ...
        for t in range(T):
            #print t
            r,a_1,b_1,c_1,d_1=self.read_attn(bgr,h_enc_prev,read_n,DO_SHARE)
            a_1=tf.reshape(a_1,(-1,1,1))
            b_1=tf.reshape(b_1,(-1,1,1))
            c_1=tf.reshape(c_1,(-1,1,1))
            d_1=tf.reshape(c_1,(-1,1,1))
           ...
           # print tf.image.resize_images(bgr[:,:,:,0],(read_n,read_n)).get_shape
            ...
            if t==0:
                 self.image_show=rr
                 self.location=tf.concat(1,[a_1,b_1,c_1,d_1])
            else:
                 self.image_show=tf.concat(3,[self.image_show,rr])
                 self.location=tf.concat(2,[self.location,tf.concat(1,[a_1,b_1,c_1,d_1])])
            print r.get_shape
    ...
    def attn_window(self,scope,h_dec,N,DO_SHARE):
        ...
        return self.filterbank(gx,gy,sigma2,delta,N)+(tf.exp(log_gamma),gx,gy,tf.sqrt(sigma2),delta)
    def read_attn(self,x,h_enc_prev,read_n,DO_SHARE):
     
        Fx,Fy,gamma,gx,gy,sigma,delta=self.attn_window("read",h_enc_prev,read_n,DO_SHARE)
        ...
        return res,gx,gy,sigma,delta # concat along feature axis
